...
export const Q9Booklet: React.FC<Q9BookletProps> = ({ answers, setAnswers, onSubmit, submitting, studentName, submitDate, isStudent, compRecord: externalCompRecord, setCompRecord: externalSetCompRecord }) => {
  ...
  return (
    <div className="q9-booklet-view">
      ...
      <div className="page">
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '5px' }}>
          <tbody>
            ...
            <tr><td className="font-bold">Assessment Task 1</td><td className="text-center">S / NS</td><td rowSpan={3} style={{ verticalAlign: 'middle' }}><div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}><label><input type="checkbox" disabled /> Competent (C)</label><label><input type="checkbox" disabled /> Not Competent (NC)</label></div></td></tr>
      ...
      {/* ═══════════════════ PAGE 4 – ADMIN (plagiarism through competency decision) ═══════════════════ */}
      <div className="page">
        <InnerHeader />
        <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '6px' }}>
          <tbody>
            <tr><td style={{ border: '1px solid #555', padding: '3px 6px', fontWeight: 'bold', width: '30%' }}>Plagiarism</td><td style={{ border: '1px solid #555', padding: '3px 6px', whiteSpace: 'pre-wrap' }}>{admin.plagiarism}</td></tr>
      ...
      <div className="page">
        ...
        <table className="w-full border-collapse border-[1.5px] border-black text-[9pt]">
          ...
          <tbody>
            ...
            {renderChkRows('task1', task1.checklistItems, 0, 8)}
          </tbody>
        </table>
        <PageFooter n={7} />
      </div>
      ...
      <div className="page">
        ...
        <table className="w-full border-collapse border-[1.5px] border-black text-[9pt] mb-4">
          ...
          <tbody>
            <tr><td colSpan={3} className="border-[1.5px] border-black bg-[#e0e0e0] font-bold px-3 py-2">Evidence of Performance: Did The Candidate Satisfactorily:</td></tr>
            {renderChkRows('task1', task1.checklistItems, 8, task1.checklistItems.length)}
          </tbody>
        </table>
      ...
      <div className="page">
        <InnerHeader />
        <h2 className="sub-title" style={{ marginBottom: '8px' }}>Equipment Required — Images</h2>
        {task2.sections?.filter((s: any) => s.type === 'image').map((s: any, i: number) => (
          <div key={i} className="mb-4 text-center">
            <img src={s.src} alt={s.caption || 'Equipment'} style={{ maxWidth: '100%', maxHeight: '200px', objectFit: 'contain', margin: '0 auto' }} />
            {s.caption && <p className="text-[9pt] italic mt-2">{s.caption}</p>}
          </div>
        ))}
        <PageFooter n={10} />
      </div>
      ...
      {/* ═══════════════════ PAGE 12 – TASK 2: checklistItems[12-end] + declarations ═══════════════════ */}
      <div className="page">
        <InnerHeader />
