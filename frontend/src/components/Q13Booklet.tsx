...
export const Q13Booklet: React.FC<Q13BookletProps> = ({ answers, setAnswers, onSubmit, submitting, studentName, submitDate, isStudent }) => {
  ...
  const q13Styles = `
      ...
      .q13-booklet-view .question-block {
        margin-bottom: 8mm;
      }
      .q13-booklet-view .question-text {
        font-weight: bold;
        margin-bottom: 3mm;
      }
      @media print {
  ...
  return (
    <div className="q13-booklet-view">
      ...
      {/* ADMIN INFO PAGES */}
      {assessmentQuestions.adminInfo?.markingGuide && (
        <div className="page">
          <div className="inner-header">
            <div className="top-row">
          ...
          {assessmentQuestions.adminInfo.markingGuide.map((guide: any, idx: number) => (
            <div key={idx} className="mb-4">
              <h3 className="font-bold text-sm mb-1">{guide.label}</h3>
              <p className="whitespace-pre-wrap text-sm">{guide.content}</p>
            </div>
          ))}
        </div>
      )}

      {assessmentQuestions.adminInfo?.tasksOverview && (
        <div className="page">
          <div className="inner-header">
            <div className="top-row">
              <div><span className="underline-bold">Assessment book</span><br /><span className="underline-bold">{assessmentQuestions.metadata.code} {assessmentQuestions.metadata.course}</span></div>
          ...
          <h1 className="section-title text-center text-blue-900 font-bold my-4">{assessmentQuestions.adminInfo.tasksOverview.title}</h1>
          <p className="whitespace-pre-wrap text-sm mb-2">{assessmentQuestions.adminInfo.tasksOverview.intro}</p>
          <ul className="list-disc pl-5 mb-4 text-sm">
            {assessmentQuestions.adminInfo.tasksOverview.elements?.map((el: string, idx: number) => (
              <li key={idx}>{el}</li>
            ))}
          </ul>
          <p className="whitespace-pre-wrap text-sm mb-2">{assessmentQuestions.adminInfo.tasksOverview.evidenceIntro}</p>
          <ul className="list-disc pl-5 mb-4 text-sm">
            {assessmentQuestions.adminInfo.tasksOverview.evidenceItems?.map((el: string, idx: number) => (
              <li key={idx}>{el}</li>
            ))}
          </ul>
          <p className="whitespace-pre-wrap text-sm mb-4">{assessmentQuestions.adminInfo.tasksOverview.summary}</p>
          <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '6px' }}>
            <tbody>
              {assessmentQuestions.adminInfo.tasksOverview.tasks?.map((task: any, idx: number) => (
                <tr key={idx}>
                  <td style={{ border: '1px solid #555', padding: '3px 6px', fontWeight: 'bold', width: '25%' }}>{task.id}<br />({task.type})</td>
                  <td style={{ border: '1px solid #555', padding: '3px 6px', whiteSpace: 'pre-wrap' }}>{task.text}</td>
                </tr>
              ))}
            </tbody>
          </table>
