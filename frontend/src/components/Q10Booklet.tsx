...
const PageFooter = ({ n }: { n: number }) => (
  <div className="page-footer"><span></span><span>Page {n} of 25</span></div>
);
...
export const Q10Booklet: React.FC<Q10BookletProps> = ({ answers, setAnswers, onSubmit, submitting, studentName, submitDate, isStudent, compRecord: externalCompRecord, setCompRecord: externalSetCompRecord }) => {
        ...
        <div className="p-3">
          ...
          {q.image && (
            <div className="flex flex-col items-center gap-2 my-2">
              ...
              {q.imageCaption && <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{q.imageCaption}</span>}
            </div>
          )}
          <div className="pl-0 mt-2">
            ...
            {q.type === 'radio' && q.options?.map((opt: any, oIdx: number) => (
              <div key={oIdx} className="flex gap-2 mb-2 items-center">
                <input type="radio" checked={answers[opt.name || qKey] === opt.value} onChange={() => setAnswers({ ...answers, [opt.name || qKey]: opt.value })} />
            ...
            {q.type === 'table' && (
              <div className="overflow-x-auto rounded-xl border border-slate-200 shadow-sm bg-white mt-2 mb-2">
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-slate-50 border-b border-slate-200">
                      {q.headers?.map((header: string, hIdx: number) => (
                        <th key={hIdx} className="p-3 text-[10px] font-bold text-slate-600 uppercase tracking-wider border border-slate-200">{header}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {q.rows?.map((row: any, rIdx: number) => (
                      <tr key={rIdx} className="border-b border-slate-100">
                        <td className="p-3 text-sm text-slate-700 font-bold bg-slate-50/30 w-1/4 border border-slate-200">{row.label}</td>
                        {row.cells ? row.cells.map((cell: any, cIdx: number) => {
                          const ans = answers[cell.name];
                          return (
                            <td key={cIdx} className="p-3 border border-slate-200 align-top">
                                  ...
                                  return (
                                    <div key={oIdx} onClick={() => {
                                      ...
                                      if (cell.type === 'checkbox') {
                                        let newArr = [...(Array.isArray(answers[cell.name]) ? answers[cell.name] : [])];
                                        if (!isSelected) newArr.push(opt.value); else newArr = newArr.filter((v: any) => v !== opt.value);
                                        newAns = newArr;
                                      } else { newAns = opt.value; }
                                      setAnswers({ ...answers, [cell.name]: newAns });
                                    }} className={`flex items-center gap-2 p-1.5 rounded-lg border cursor-pointer transition-all text-[10px] ${isSelected ? 'bg-[#1e3a8a] text-white border-[#1e3a8a] font-bold' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'}`}>
                                      {opt.text}
                                    </div>
                                  );
                                }) : (
                                  <textarea className="w-full bg-transparent outline-none resize-y min-h-[40px] text-[10px]" placeholder="(Enter details here)" value={ans || ''} onChange={(e) => setAnswers({ ...answers, [cell.name]: e.target.value })} />
                                )}
                              </div>
                            </td>
                          );
                        }) : (
                          <td className="p-3 border border-slate-200" colSpan={q.headers.length - 1}>
                            <input type="text" className="w-full p-2 border border-slate-200 rounded text-sm outline-none" placeholder="Enter answer..." value={answers[row.id] || ''} onChange={(e) => setAnswers({ ...answers, [row.id]: e.target.value })} />
                          </td>
                        )}
                      </tr>
                    ))}
            ...
            {q.type === 'jsa_table' && (
              <div className="mt-2 border-2 border-slate-200 rounded-xl overflow-hidden bg-white shadow-md mb-2">
                <div className="grid grid-cols-2 md:grid-cols-4 border-b border-slate-200">
                  {q.fields?.slice(0, 4).map((f: any, idx: number) => (
                    <div key={idx} className="p-2 border-r border-slate-100 last:border-r-0">
                      <label className="block text-[9px] font-black text-slate-400 uppercase tracking-tighter mb-1">{f.label}</label>
                      <input className="text-sm font-bold text-[#1e3a8a] w-full bg-transparent outline-none" value={answers[f.name] || ''} onChange={(e) => setAnswers({ ...answers, [f.name]: e.target.value })} placeholder="..." />
                    </div>
                  ))}
                </div>
                <div className="grid grid-cols-2 md:grid-cols-3 border-b border-slate-200">
                  {q.fields?.slice(4, 10).map((f: any, idx: number) => (
                    <div key={idx} className="p-2 border-r border-b border-slate-100 last:border-r-0">
                      <label className="block text-[9px] font-black text-slate-400 uppercase tracking-tighter mb-1">{f.label}</label>
                      <input className="text-sm font-bold text-[#1e3a8a] w-full bg-transparent outline-none" value={answers[f.name] || ''} onChange={(e) => setAnswers({ ...answers, [f.name]: e.target.value })} placeholder="..." />
                    </div>
                  ))}
                  ...
                  <thead>
                    <tr className="bg-slate-800 text-white">
                      {q.steps?.headers?.map((h: string, idx: number) => (
                        <th key={idx} className="p-2 text-xs font-bold uppercase tracking-widest border-r border-slate-700 last:border-0">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {[...Array(q.steps?.rowCount || 5)].map((_, rIdx) => (
                      <tr key={rIdx} className="border-t border-slate-200">
                        {[0, 1, 2].map((cIdx) => {
                          const stepKey = `${qKey}_r${rIdx}c${cIdx}`;
                          return (
                            <td key={cIdx} className="p-2 border-r border-slate-200 last:border-0 align-top">
                              <textarea className="p-1 w-full bg-blue-50/50 border border-blue-100 rounded text-slate-800 text-[12px] min-h-[50px] italic outline-none resize-y" value={answers[stepKey] || ''} onChange={(e) => setAnswers({ ...answers, [stepKey]: e.target.value })} placeholder="(Empty)" />
                            </td>
                          );
                        })}
                      </tr>
  ...
  return (
    <div className="q10-booklet-view">
      ...
      <div className="page">
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '5px' }}>
          <tbody>
            ...
            <tr><td className="font-bold">Assessment Task 1: Case Study</td><td className="text-center">S / NS</td><td rowSpan={4} style={{ verticalAlign: 'middle' }}><div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}><label><input type="checkbox" disabled /> Competent (C)</label><label><input type="checkbox" disabled /> Not Competent (NC)</label></div></td></tr>
            <tr><td className="font-bold">Assessment Task 2: Knowledge Evidence</td><td className="text-center">S / NS</td></tr>
            <tr><td className="font-bold">Assessment Task 3: Written Q &amp; A</td><td className="text-center">S / NS</td></tr>
            <tr><td className="font-bold">Assessment Task 4: Additional Worksheet</td><td className="text-center">S / NS</td></tr>
          </tbody>
        </table>
        <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '5px' }}>
      ...
      {/* ═══════════════════ PAGE 11 – TASK 1 ASSESSOR CHECKLIST: items 0-9 ═══════════════════ */}
      <div className="page">
        <InnerHeader />
        ...
        <table className="w-full border-collapse border-[1.5px] border-black text-[9pt]">
          ...
          <tbody>
            ...
            {renderChkRows('task1', task1.checklistItems, 0, 10)}
          </tbody>
        </table>
        <PageFooter n={11} />
      </div>

      {/* ═══════════════════ PAGE 12 – TASK 1: checklist items 10-end + declarations ═══════════════════ */}
      <div className="page">
        <InnerHeader />
        ...
        <table className="w-full border-collapse border-[1.5px] border-black text-[9pt] mb-4">
          ...
          <tbody>
            <tr><td colSpan={3} className="border-[1.5px] border-black bg-[#e0e0e0] font-bold px-3 py-2">Evidence of Performance: Did The Candidate Satisfactorily:</td></tr>
            {renderChkRows('task1', task1.checklistItems, 10, task1.checklistItems.length)}
          </tbody>
        </table>
      ...
      {/* ═══════════════════ PAGE 13 – TASK 2: Knowledge Evidence sections + Q1-2 ═══════════════════ */}
      <div className="page">
        <InnerHeader />
        ...
        <div>{task2.questions?.map((q: any) => renderQ(q, 'task2'))}</div>
        <PageFooter n={13} />
      </div>

      {/* ═══════════════════ PAGE 14 – TASK 2 ASSESSOR CHECKLIST: oral/knowledge questions (0-9) ═══════════════════ */}
      <div className="page">
        <InnerHeader />
        ...
        <h3 className="font-bold mb-2 text-[10pt]">Knowledge Evidence Questions:</h3>
        <table className="w-full border-collapse border-[1.5px] border-black text-[9pt]">
          <ChkHead />
          <tbody>
      ...
      {/* ═══════════════════ PAGE 15 – TASK 2: performance items (10-end) + declarations ═══════════════════ */}
      <div className="page">
        <InnerHeader />
        ...
        <table className="w-full border-collapse border-[1.5px] border-black text-[9pt] mb-4">
          <ChkHead />
          <tbody>
            <tr><td colSpan={3} className="border-[1.5px] border-black bg-[#e0e0e0] font-bold px-3 py-2">Did The Candidate Satisfactorily:</td></tr>
      ...
      <div className="page">
        ...
        <div>{task3.questions?.slice(0, 2).map((q: any) => renderQ(q, 'task3'))}</div>
        <PageFooter n={16} />
      </div>
      ...
      <div className="page">
        <InnerHeader />
        <div>{task3.questions?.slice(2, 7).map((q: any) => renderQ(q, 'task3'))}</div>
        <PageFooter n={17} />
      </div>

      {/* ═══════════════════ PAGE 18 – TASK 3: Q8-10 ═══════════════════ */}
      <div className="page">
        <InnerHeader />
        <div>{task3.questions?.slice(7).map((q: any) => renderQ(q, 'task3'))}</div>
        <PageFooter n={18} />
      </div>
      ...
      <div className="page">
        <InnerHeader />
        <h1 className="section-title">{task4.title}</h1>
        <div>{task4.questions?.slice(0, 4).map((q: any) => renderQ(q, 'task4'))}</div>
        <PageFooter n={20} />
      </div>

      {/* ═══════════════════ PAGE 21 – TASK 4: Q5-10 ═══════════════════ */}
      <div className="page">
        <InnerHeader />
        <div>{task4.questions?.slice(4, 10).map((q: any) => renderQ(q, 'task4'))}</div>
        <PageFooter n={21} />
      </div>

      {/* ═══════════════════ PAGE 22 – TASK 4: Q11-16 ═══════════════════ */}
      <div className="page">
        <InnerHeader />
        <div>{task4.questions?.slice(10, 16).map((q: any) => renderQ(q, 'task4'))}</div>
        <PageFooter n={22} />
      </div>
      ...
      <div className="page">
        <InnerHeader />
        <div>{task4.questions?.slice(16, 22).map((q: any) => renderQ(q, 'task4'))}</div>
        <PageFooter n={23} />
      </div>

      {/* ═══════════════════ PAGE 24 – TASK 4: Q23-28 ═══════════════════ */}
      <div className="page">
        <InnerHeader />
        <div>{task4.questions?.slice(22, 28).map((q: any) => renderQ(q, 'task4'))}</div>
        <PageFooter n={24} />
      </div>

      {/* ═══════════════════ PAGE 25 – TASK 4: declarations + end of assessment ═══════════════════ */}
      <div className="page">
        <InnerHeader />
        ...
        <div style={{ marginTop: '20px', border: '1px solid #555', padding: '8px' }}>
          ...
          <ol style={{ paddingLeft: '20px', fontSize: '9.5pt', lineHeight: '1.6' }}>
            ...
            <li>Write your Name and Student ID on the first page and sign the student declaration</li>
