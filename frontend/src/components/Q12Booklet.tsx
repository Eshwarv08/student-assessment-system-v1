...
export const Q12Booklet: React.FC<Q12BookletProps> = ({ answers, setAnswers, onSubmit, submitting, studentName, submitDate, isStudent, compRecord: externalCompRecord, setCompRecord: externalSetCompRecord }) => {
  ...
  const renderQ = (q: any, taskKey: string) => {
    ...
    return (
      <div key={q.id} className="mb-4 border-[1.5px] border-black bg-white flex flex-col">
        <div className="p-3">
          <div className="flex gap-2 font-bold mb-2 text-[10pt]"><span className="whitespace-pre-wrap">{q.text}</span></div>
  ...
  return (
    <div className="q12-booklet-view">
      ...
      <div className="page">
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '5px' }}>
          <tbody>
            ...
            <tr><td className="font-bold">Assessment Task 4: Questions and Answers</td><td className="text-center">S / NS</td></tr>
          </tbody>
        </table>
        <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '5px' }}>
            ...
            <tr>
              ...
              <td style={{ border: '1px solid #555', padding: '5px 8px', fontSize: '8.5pt', width: '50%', verticalAlign: 'top' }}>
                <div className="flex items-center gap-2">Signature:
                  <div onClick={() => openSigModal('assessor_signature', 'comp')} style={{ flex: 1, cursor: isStudent ? 'default' : 'pointer', borderBottom: '1px solid #000', minHeight: '24px', display: 'inline-flex', alignItems: 'center', paddingLeft: '4px' }}>
                    {compRecord.assessor_signature ? <img src={compRecord.assessor_signature} className="max-h-[25px] max-w-[100px] object-contain" /> : <span style={{ fontSize: '10px', color: '#aaa', fontStyle: 'italic' }}>Sign Here</span>}
                  </div>
                </div>
                <div className="mt-2">Date: <span style={{ fontWeight: 'bold', borderBottom: '1px dashed #aaa', display: 'inline-block', minWidth: '80px', textAlign: 'center', marginLeft: '4px' }}>{formatDisplayDate(compRecord.assessor_sig_date)}</span></div>
              </td>
            </tr>
            <tr>
              ...
              <td style={{ border: '1px solid #555', padding: '5px 8px', fontSize: '8.5pt', verticalAlign: 'top' }}>
                <div className="flex items-center gap-2">Signature:
                  <div onClick={() => openSigModal('student_signature', 'comp')} style={{ flex: 1, cursor: 'pointer', borderBottom: '1px solid #000', minHeight: '24px', display: 'inline-flex', alignItems: 'center', paddingLeft: '4px' }}>
                    {answers.student_signature_url ? <img src={answers.student_signature_url} className="max-h-[25px] max-w-[100px] object-contain" /> : <span style={{ fontSize: '10px', color: '#aaa', fontStyle: 'italic' }}>Sign Here</span>}
                  </div>
                </div>
                <div className="mt-2">Date: <span style={{ fontWeight: 'bold' }}>{formatDisplayDate(submitDate || '')}</span></div>
              </td>
            </tr>
          </tbody>
        </table>
      ...
      {/* ═══════════════════ PAGE 6 – TASK 1 CHECKLIST: observationItems + checklistItems[0-6] ═══════════════════ */}
      <div className="page">
        <InnerHeader />
      ...
      {/* ═══════════════════ PAGE 7 – TASK 1: checklistItems[7-end] + declarations ═══════════════════ */}
      <div className="page">
        <InnerHeader />
      ...
      <div className="page">
        ...
        {task2.sections?.map((s: any, i: number) => (
          <div key={i} className="mb-3">
            {s.title && <h3 className="font-bold mb-1">{s.title}</h3>}
            {s.type === 'image' ? (
              ...
              <p className="whitespace-pre-wrap text-[9pt]">{s.content}</p>
            )}
          </div>
        ))}
        <PageFooter n={8} />
      ...
      {/* ═══════════════════ PAGE 9 – TASK 2 CHECKLIST: observationList + checklistItems[0-11] ═══════════════════ */}
      <div className="page">
        <InnerHeader />
        ...
        <table className="w-full border-collapse border-[1.5px] border-black text-[9pt]">
          <ChkHead />
          <tbody>
            <tr><td colSpan={3} className="border-[1.5px] border-black bg-[#e0e0e0] font-bold px-3 py-2">Observed Tasks:</td></tr>
            {renderObsRows('task2', task2.observationList || [])}
            <tr><td colSpan={3} className="border-[1.5px] border-black bg-[#e0e0e0] font-bold px-3 py-2">Evidence of Performance: Did The Candidate Satisfactorily:</td></tr>
            {renderChkRows('task2', task2.checklistItems, 0, 12)}
          </tbody>
        </table>
        <PageFooter n={9} />
      </div>

      {/* ═══════════════════ PAGE 10 – TASK 2: checklistItems[12-end] + declarations ═══════════════════ */}
      <div className="page">
        <InnerHeader />
        ...
        <table className="w-full border-collapse border-[1.5px] border-black text-[9pt] mb-4">
          ...
          <tbody>
            <tr><td colSpan={3} className="border-[1.5px] border-black bg-[#e0e0e0] font-bold px-3 py-2">Evidence of Performance: Did The Candidate Satisfactorily:</td></tr>
            {renderChkRows('task2', task2.checklistItems, 12, task2.checklistItems.length)}
          </tbody>
        </table>
      ...
      {/* ═══════════════════ PAGE 12 – TASK 3 CHECKLIST: observationItems + checklistItems[0-10] ═══════════════════ */}
      <div className="page">
        <InnerHeader />
        ...
        <table className="w-full border-collapse border-[1.5px] border-black text-[9pt]">
          ...
          <tbody>
            ...
            <tr><td colSpan={3} className="border-[1.5px] border-black bg-[#e0e0e0] font-bold px-3 py-2">Evidence of Performance: Did The Candidate Satisfactorily:</td></tr>
            {renderChkRows('task3', task3.checklistItems, 0, 11)}
          </tbody>
        </table>
        <PageFooter n={12} />
      </div>

      {/* ═══════════════════ PAGE 13 – TASK 3: checklistItems[11-end] + declarations ═══════════════════ */}
      <div className="page">
        <InnerHeader />
        ...
        <table className="w-full border-collapse border-[1.5px] border-black text-[9pt] mb-4">
          ...
          <tbody>
            <tr><td colSpan={3} className="border-[1.5px] border-black bg-[#e0e0e0] font-bold px-3 py-2">Evidence of Performance: Did The Candidate Satisfactorily:</td></tr>
            {renderChkRows('task3', task3.checklistItems, 11, task3.checklistItems.length)}
          </tbody>
        </table>
      ...
      <div className="page">
        <InnerHeader />
        <div>{task4.questions?.slice(4).map((q: any) => renderQ(q, 'task4'))}</div>
        <PageFooter n={15} />
      </div>
