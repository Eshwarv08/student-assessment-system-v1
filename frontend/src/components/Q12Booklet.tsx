...
export const Q12Booklet: React.FC<Q12BookletProps> = ({ answers, setAnswers, onSubmit, submitting, studentName, submitDate, isStudent }) => {
  ...
  const q12Styles = `
      ...
      .q12-booklet-view h2.sub-title {
        font-size: 11pt;
        font-weight: bold;
        text-align: center;
        margin: 2mm 0;
      }
      .q12-booklet-view h3.task-label {
        font-size: 10.5pt;
        font-weight: bold;
        text-align: center;
        margin: 1mm 0 3mm;
      }
      .q12-booklet-view .intro-box {
        background: #f5f5f5;
      ...
      .q12-booklet-view table td, .q12-booklet-view table th {
        border: 1px solid #555;
        padding: 3px 6px;
        vertical-align: top;
      }
      .q12-booklet-view table th {
        background: #e8e8e8;
        font-weight: bold;
      }
      .q12-booklet-view .field-label-cell {
        font-weight: bold;
        background: #f0f0f0;
        width: 38%;
        border: 1px solid #555;
      ...
      .q12-booklet-view .field-value-cell {
        border: 1px solid #555;
        padding: 5px 6px;
        min-height: 22px;
      }
      ...
      .q12-booklet-view .evidence-item { display: flex; align-items: center; gap: 4px; }
      .q12-booklet-view .result-badge {
      ...
      .q12-booklet-view .inner-header {
        margin-bottom: 4mm;
        border-bottom: 2px solid #000;
        padding-bottom: 2mm;
      }
      .q12-booklet-view .inner-header .top-row {
        display: flex; justify-content: space-between; align-items: flex-start;
      }
      .q12-booklet-view .inner-header .title-block { font-weight: bold; font-size: 11.5pt; color: #b00; }
      .q12-booklet-view .underline-bold { text-decoration: underline; font-weight: bold; }
      .q12-booklet-view .checklist-table th { background: #e0e0e0; font-size: 9.5pt; }
      .q12-booklet-view .checklist-table td { padding: 4px 6px; font-size: 9pt; }
      .q12-booklet-view .question-block {
        margin-bottom: 8mm;
      }
      .q12-booklet-view .question-text {
        font-weight: bold;
        margin-bottom: 3mm;
      }
      @media print {
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '6px' }}>
          <tbody>
            ...
            {assessmentQuestions.adminInfo.taskOverviews?.map((task: any, idx: number) => (
              <tr key={idx}><td style={{ border: '1px solid #555', padding: '3px 6px', fontWeight: 'bold' }}>{task.id.replace(':', '')}</td><td style={{ border: '1px solid #555', padding: '3px 6px', whiteSpace: 'pre-wrap' }}>{task.text}</td></tr>
            )) || (
              <>
                <tr><td style={{ border: '1px solid #555', padding: '3px 6px', fontWeight: 'bold' }}>Assessment Task 1</td><td style={{ border: '1px solid #555', padding: '3px 6px', whiteSpace: 'pre-wrap' }}>{(assessmentQuestions.adminInfo as any).task1Description}</td></tr>
                <tr><td style={{ border: '1px solid #555', padding: '3px 6px', fontWeight: 'bold' }}>Assessment Task 2</td><td style={{ border: '1px solid #555', padding: '3px 6px', whiteSpace: 'pre-wrap' }}>{(assessmentQuestions.adminInfo as any).task2Description}</td></tr>
                <tr><td style={{ border: '1px solid #555', padding: '3px 6px', fontWeight: 'bold' }}>Assessment Task 3</td><td style={{ border: '1px solid #555', padding: '3px 6px', whiteSpace: 'pre-wrap' }}>{(assessmentQuestions.adminInfo as any).task3Description}</td></tr>
              </>
            )}
          </tbody>
        </table>
        ...
        .map((taskKey, idx) => {
          ...
          return (
            <div key={taskKey} className="page">
                      ...
                      <div className="p-3 sm:p-4">
                        ...
                        {q.image && (
                          <div className="flex flex-col items-center gap-2 mt-3 mb-2 px-2">
                            <div className={`bg-white p-1 border border-slate-200 shadow-sm rounded-lg w-full ${q.smallImage ? 'max-w-[300px]' : 'max-w-[600px]'}`}>
                              <img src={q.image} alt={q.imageCaption || `Question ${q.id} diagram`} className="w-full h-auto rounded" />
                            </div>
                            {q.imageCaption && (
                              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{q.imageCaption}</span>
                            )}
                          </div>
                        )}
                              ...
                              <>
                                {q.type === 'radio' && q.options?.map((opt: any, oIdx: number) => (
                                  <div key={oIdx} className="flex gap-2 mb-2 items-center">
                                    <input type="radio" checked={answers[opt.name || qKey] === opt.value} onChange={(e) => setAnswers({...answers, [opt.name || qKey]: opt.value})} className="mt-0.5" />
                                ...
                                {q.type === 'table' && (
                                  <div className="overflow-x-auto rounded-xl border border-slate-200 shadow-sm bg-white mt-4 mb-4">
                                                    ...
                                                    <div className="flex flex-col gap-1.5">
                                                      {cell.options ? cell.options.map((opt: any, oIdx: number) => {
                                                        ...
                                                        return (
                                                          ...
                                                          }} className={`flex items-center gap-2 p-1.5 rounded-lg border cursor-pointer transition-all ${isSelected ? 'bg-[#1e3a8a] text-white border-[#1e3a8a] font-bold shadow-sm' : 'bg-white border-slate-50 text-slate-400 opacity-60 hover:opacity-100'}`}>
                                ...
                                {q.type === 'jsa_table' && (
                                  <div className="mt-4 border-2 border-slate-200 rounded-xl overflow-hidden bg-white shadow-md mb-4">
                                    {/* Top metadata grid */}
                                    <div className="grid grid-cols-1 md:grid-cols-4 border-b border-slate-200">
                                      {q.fields?.slice(0, 4).map((f: any, idx: number) => (
                                        <div key={idx} className={`p-3 border-r border-slate-100 last:border-r-0 ${idx === 3 ? 'bg-slate-50' : ''}`}>
                                          ...
                                          <input className="text-sm font-bold text-[#1e3a8a] w-full bg-transparent outline-none hover:bg-slate-100 focus:bg-white p-1 -m-1 rounded" value={answers[f.name] || ''} onChange={(e) => setAnswers({ ...answers, [f.name]: e.target.value })} placeholder="(Not provided)" />
                                        </div>
                                      ))}
                                    </div>
              ...
              {(taskData.performance || taskData.oral || taskData.checklistItems || taskData.observationItems || taskData.observationList) && (
                <div className="mt-8">
                  ...
                  <table className="w-full border-collapse border-[1.5px] border-black mb-6 text-[9pt]">
                    ...
                    <tbody>
                      ...
                      {(taskData.observationItems || taskData.observationList) && (
                        <>
                          <tr>
                            <td colSpan={3} className="border-[1.5px] border-black bg-[#e0e0e0] font-bold px-3 py-2 text-black">
                              Observation Items:
                          ...
                          {(taskData.observationItems || taskData.observationList).map((item: string, idx: number) => (
                            <tr key={`obs-${idx}`}>
                              <td className="border-[1.5px] border-black px-3 py-2">{item}</td>
