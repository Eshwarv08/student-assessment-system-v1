...
export const Q2Booklet: React.FC<Q2BookletProps> = ({ answers, setAnswers, onSubmit, submitting, studentName, submitDate }) => {
  ...
  // The signature pad logic is handled by the main AssessmentForm, so we provide dummy methods for the assessor sig modals
  const [sigModal, setSigModal] = useState<any>(null);
  const openSigModal = (field: string, type: string) => {};
  const closeSigModal = () => {};
  const clearSig = () => {};
  const saveSignature = () => {};
  const sigModalCanvasRef = null;
  const sigModalContainerRef = null;
              ...
              <tr>
                ...
                <td style={{ border: '1px solid #777', padding: '8px 12px', width: '50%', verticalAlign: 'top' }}>
                  ...
                  <div className="space-y-1 text-red-600 font-bold text-xs italic">
                    ...
                    <div className="flex items-center gap-1">M/M 62.5/125: <input className="w-16 bg-transparent border-b border-gray-400 outline-none text-center" value={answers['t4q6_d'] || answers['t4q6_a'] || ''} onChange={(e) => setAnswers({ ...answers, 't4q6_d': e.target.value })} /> microns</div>
                  </div>
                </td>
              </tr>
              <tr>
